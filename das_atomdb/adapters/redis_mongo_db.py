...
class RedisMongoDB(IAtomDB):
    ...
    def _connection_mongo_db(self) -> Database:
        ...
        try:
            global_bundle = "global-bundle.pem"
            self.mongo_db = MongoClient(
                f"mongodb://{mongo_username}:{mongo_password}@{mongo_hostname}:{mongo_port}?tls=true&tlsCAFile={global_bundle}&replicaSet=rs0&readPreference=secondaryPreferred&retryWrites=false"
            )[self.database_name]
